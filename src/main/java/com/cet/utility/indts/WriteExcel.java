...
public class WriteExcel {
	...
	public static void deletViewUser() throws Exception {
		Boolean flag=ExcelUtility.deleteRows(excelPath,"viewUser"); 
		System.out.println("Delete Operation Completed ::"+flag);
	}

	public static void deletetaskdetails() throws Exception {
		Boolean flag=ExcelUtility.deleteRows(excelPath,"indtstaskedit"); 
		System.out.println("Delete Operation Completed ::"+flag);
	}
	...
	public static void viewUserWriteTest() throws Exception {
		...
		String query = "select u.msid,u.firstname,u.lastname,u.EmpId,u.Email,u.SupervisorFirstName,\r\n" + 
				...
				"on lookupmasterid = SkillLkup where u.isactive = 1 and s.isactive = 1 and skilllkup = 1 \r\n" + 
				"order By  u.msid  OFFSET 8 ROWS FETCH NEXT 1 ROWS ONLY";
		Connection conn = DBConnection.getConnection();
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery(query);
		String sheetName = "viewUser";
	...
	public static void getStatusInActiveMSID() throws Exception {
		String query = "select msid as inactiveMsid from dbo.AppUser  where status=0  order By  msid OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY";
	...
	public static void getStatusActiveMSID() throws Exception {
		String query = "select msid as inactiveMsid from dbo.AppUser  where status=1  order By  msid OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY";
		String sheetName = "editUser";
		ExcelUtility.setExcelFile(excelPath, sheetName);
		Connection conn = DBConnection.getConnection();
		Statement stmt = conn.createStatement();
	...
	public static void getChecekdWorkedMSID() throws Exception {
		String query = "select msid as unchekmsid from dbo.AppUser  where IsAssignTask=1  order By  msid OFFSET 8 ROWS FETCH NEXT 1 ROWS ONLY";
		String sheetName = "editUser";
		ExcelUtility.setExcelFile(excelPath, sheetName);
		Connection conn = DBConnection.getConnection();
		Statement stmt = conn.createStatement();
	...
	public static void getEnglishUnchekedMsID() throws Exception {
		String query = "select u.msid from [dbo].[AppUser] u\r\n" + 
				...
				"u.isactive = 1 and s.isactive = 1 and Status = 1 and userId not in (select userRef from dbo.AppUserSkill where Skilllkup = 5)\r\n" + 
	...
	public static void getUnChecekdWorkedMSID() throws Exception {
		String query = "select msid as unchekmsid from dbo.AppUser  where  IsAssignTask=0  order By  msid OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY";
		String sheetName = "editUser";
		ExcelUtility.setExcelFile(excelPath, sheetName);
		Connection conn = DBConnection.getConnection();
		Statement stmt = conn.createStatement();
	...
	public static String getlabel(String label) throws Exception {
		String value="";
		switch(label) {
	...
	@Test
	public static void indtstaskEdit() throws Exception {
		deletetaskdetails();
		String query = "select bConnID,CampaignDescription,Jobname,CurrentPlan,PlanToPromote,i.FirstName as FirstName ,i.LastName as LastName, Address,City,State \r\n" + 
				",ZipCode,County,PrimaryPhone,PhoneNum2,PhoneNum3,Language,MBINum,MedicaidNumber,VerificationStatus,ProviderInformation\r\n" + 
				...
				"concat(u.lastname,', ',u.firstname) as AgentAssigned, concat(u.SupervisorLastName,', ',u.SupervisorFirstName) as AgentSupervisor, other, c.currentattemptcount,c.Notes\r\n" + 
				"from [AppIndividualTS] i\r\n" + 
				...
				"where taskstatuslkup = 31 and c.isactive =1 and assignedto = 114 and tasktype = 'INDTS'\r\n" + 
				"order By  bconnid OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY";
		Connection conn = DBConnection.getConnection();
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery(query);
		String sheetName = "indtstaskedit";
		ExcelUtility.setExcelFile(excelPath, sheetName);
		rowc = ExcelUtility.initialGetRowCount();
		initColc=ExcelUtility.getCellCount(0);
		while (rs.next()) { 
	...
	public static String getindtstasklabel(String label) throws Exception {
		String value="";
		switch(label) {
	...
	public static void getIndtsTaskToSearch() throws Exception {
		Boolean flag=ExcelUtility.deleteRows(excelPath,"indtstaskSearch"); 
		System.out.println("Delete Operation Completed ::"+flag);
		String query = "select ait.bconnid, tasktype, campaignfocus, m.lookupvalue as taskstatus, Jobname, dialingmethod, AIT.firstname, AIT.lastname, primaryphone,\r\n" + 
				...
				"Inner Join dbo.AppIndividualTS AIT ON ACT.AppIndividualTSRef = AIT.AppIndividualTSID\r\n" + 
				"join appuser u on u.userid = act.assignedto\r\n" + 
				"join applookupmaster m on m.lookupmasterid = act.taskstatuslkup\r\n" + 
				"where ait.isactive = 1 and act.isactive = 1\r\n" + 
				"order By ait.bconnid OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ";
		Connection conn = DBConnection.getConnection();
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery(query);
		String sheetName = "indtstaskSearch";
		ExcelUtility.setExcelFile(excelPath, sheetName);
		rowc = ExcelUtility.initialGetRowCount();
		initColc=ExcelUtility.getCellCount(0);
		while (rs.next()) { 
